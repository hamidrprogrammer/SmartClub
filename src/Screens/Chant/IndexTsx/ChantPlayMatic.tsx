...
import { getAlldely } from "@/src/core/save";
import { useNavigation } from "@react-navigation/native";
import { Thread } from 'react-native-threads';
const torchThread = new Thread('./torchWorker.js');
export default function LyricsPageTest({  route }) {
  ...
  const runTimeouts = () => {
    let totalDuration = 0;
    const time = Platform.OS !== 'ios' ? 100 : 100;
    
    // Post data to the worker thread
    torchThread.postMessage({
      chantContent,
      time,
      platform: Platform.OS,
    });
  
    // Listen for messages from the worker thread if needed
    torchThread.onmessage = (message) => {
      // Handle the message from the worker thread (if required)
      console.log('Intervals from worker:', message.intervals);
    };
  
    // Handle any errors in the worker thread
    torchThread.onerror = (error) => {
